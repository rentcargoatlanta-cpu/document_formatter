  ...
  while (i < n) {
    ...
    if (ch === '[') {
      ...
      while (j < n && depth > 0) {
        ...
        if (c === '(') {
          // Consume balanced string literal into body.
          let pd = 1;
          body += c;
          j += 1;
          while (j < n && pd > 0) {
    ...
    if (isNumberStart(ch, next)) {
      ...
      // Not a real number — fall through to operator scan below from `i`.
    }
    ...
    // Operator: run of non-whitespace, non-delimiter characters.
    {
      ...
      if (j === i) {
        // Unrecognized delimiter we didn't handle — skip to avoid infinite loop.
        i += 1;
        continue;
      }
...
function findPlaceholderLocations(contentStream: string): PlaceholderLocation[] {
  ...
  for (const tok of tokenize(contentStream)) {
    ...
    switch (op) {
      ...
      default:
        // Unhandled operator: consume operands and move on.
        break;
    }
